...
function clearAllCacheInternal() {
    ...
    logSecurityEvent("cache_clear_requested", { sessionEmail: callerEmail });
...
function addUserToIndex(email) {
    try {
        ...
        Logger.log(
            "(addUserToIndex)添加使用者到索引時發生錯誤：%s",
            error.message,
        );
        return false;
    }
    ...
    try {
        ...
        try {
            ...
            if (newIndex.length !== userIndex.length) {
                Logger.log(
                    "(removeUserFromIndex)已從快取索引中移除使用者：%s",
                    email,
                );
            }
